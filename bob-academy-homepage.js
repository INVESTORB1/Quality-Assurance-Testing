...
document.addEventListener('DOMContentLoaded', function() {
	...
	const slideshow = document.querySelector('.feature-slideshow');
	if (slideshow && dotsContainer.children.length) slideshow.appendChild(dotsContainer);
	...
	function showFeatureSlide(index) {
		featureSlides.forEach((slide, i) => {
			...
			if (featureDots[i]) featureDots[i].classList.toggle('active', i === index);
		});
	}
